...
const EmailEditorWrapper = () => {
  const [initialTemplate, setInitialTemplate] = useState<TEditorConfiguration | null>(null);
  const [initialTemplateId, setInitialTemplateId] = useState<string | null>(null);
  const [initialTemplateName, setInitialTemplateName] = useState<string | null>(null);
  ...
  const handleSave = (template: TEditorConfiguration) => {
    try {
      ...
      if (currentId && currentName) {
        const existingTemplatesJSON = localStorage.getItem('savedTemplates');
        const existingTemplates: StoredTemplate[] = existingTemplatesJSON ? JSON.parse(existingTemplatesJSON) : [];
        const templateIndex = existingTemplates.findIndex(t => t.id === currentId);
  ...
  const handleLoadTemplate = async (templateId: string): Promise<TEditorConfiguration | null> => {
    ...
    try {
      ...
      if (template) {
        ...
        localStorage.setItem('lastEditedTemplate', JSON.stringify(template.content));
